...
CREATE TABLE IF NOT EXISTS avis (
  ...
  contenu     text NOT NULL,
  service     text,
  statut      text DEFAULT 'publié' CHECK (statut IN ('publié', 'en_attente')),
...
-- ── 5. Table PARAMÈTRES DU SITE (thème/couleurs) ────────────
...
CREATE POLICY "lecture avis publiés"
  ON avis FOR SELECT TO anon USING (statut = 'publié');
...
CREATE POLICY "admin avis"
  ON avis FOR ALL TO authenticated USING (true) WITH CHECK (true);
...
CREATE INDEX IF NOT EXISTS idx_avis_statut     ON avis (statut, date DESC);
