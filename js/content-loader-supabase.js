...
(function() {
    ...
    async function loadBlogPageContent() {
        ...
        // On injecte les articles Supabase UNIQUEMENT dans #blog-supabase-slot (en bas)
        ...
        try {
            ...
            // Injecter le titre séparateur si pas encore présent
            var container = document.getElementById('blog-posts-container');
            if (container && !document.getElementById('blog-supabase-title')) {
                var divider = document.createElement('div');
                divider.id = 'blog-supabase-title';
                divider.style.cssText = 'margin-top:48px;margin-bottom:0;padding-bottom:14px;border-bottom:2px solid var(--gray-200,#dceee4);display:flex;align-items:center;gap:14px';
                divider.innerHTML =
                    '<div style="width:4px;height:32px;background:linear-gradient(135deg,#c9a84c,#e8d080);border-radius:4px;flex-shrink:0"></div>' +
                    '<div>' +
                      '<h3 style="margin:0;font-size:1.2rem;color:var(--emerald-deep,#0c3320)">Nouveaux articles</h3>' +
                      '<p class="muted" style="margin:2px 0 0;font-size:.82rem">Publiés depuis l\'administration</p>' +
                    '</div>';
                supabaseSlot.parentNode.insertBefore(divider, supabaseSlot);
            }
            ...
            blogs.forEach(function(blog, index) {
                ...
                // Image : même structure que les cartes permanentes (ratio 16/9)
                var imgHtml = '';
                if (blog.image_url && isValidUrl(blog.image_url)) {
                    imgHtml =
                        '<div class="blog-img-wrap">' +
                          '<img src="' + esc(blog.image_url) + '"' +
                               ' alt="' + esc(blog.title) + '"' +
                ...
                var bodyEl = article.querySelector('.blog-body');
                bodyEl.appendChild(contentP);

                // Bouton partager
                var actionsDiv = document.createElement('div');
                actionsDiv.style.cssText = 'display:flex;align-items:center;justify-content:space-between;margin-top:14px;flex-wrap:wrap;gap:8px';
                actionsDiv.innerHTML =
                    '<span class="muted" style="font-size:.78rem">✨ Nouveau</span>' +
                    '<button onclick="shareArticle(\'' + esc(blog.title).replace(/'/g,"\\'") + '\')" ' +
                      'style="background:none;border:1px solid var(--gray-200,#dceee4);border-radius:8px;padding:5px 12px;font-size:.78rem;cursor:pointer;color:var(--muted,#5a7a66)">📤 Partager</button>';
                bodyEl.appendChild(actionsDiv);

                // Fermer blog-body
                var closeDiv = document.createElement('div');
                bodyEl.outerHTML = bodyEl.outerHTML; // force close — use direct append instead
                article.querySelector('.blog-body').insertAdjacentHTML('beforeend', '');

                // Animation d'entrée
                article.style.cssText = 'opacity:0;transform:translateY(20px);transition:opacity 0.5s ease,transform 0.5s ease';
                supabaseSlot.appendChild(article);
                setTimeout(function(el) {
                    el.style.opacity = '1';
                    el.style.transform = 'translateY(0)';
                }.bind(null, article), index * 120);
            });
